---
title: "Pre-processing of hyperspectral data 2021"
...
```{r}
hyperspec_2021 <- read_csv("../../raw_data/hyperspectral/reflectance_2021.csv")
...
```{r}
pre_processing_steps <- expand.grid(date = c(unique(hyperspec_2021$date)),
...
hyperspec_2021$wavelength_chr <- as.character(hyperspec_2021$wavelength) # making a character version of this column to facilitate joins

VNIR_wide <- hyperspec_2021 |> 
  filter(camera == "VNIR") |> 
...
test_savgol_param(ref_matrix = VNIR_ref_matrix, 
                  ...
                  stop_wavelength = 550,
                  original_dat = hyperspec_2021 |> filter(camera == "VNIR"))
```
...
test_savgol_param(ref_matrix = SWIR_ref_matrix, 
                  ...
                  original_dat = hyperspec_2021 |> filter(camera == "SWIR"))
```
...
VNIR_smoothed_out <- hyperspec_2021 |> 
  filter(camera == "VNIR") |> 
...
SWIR_smoothed_out <- hyperspec_2021 |> 
  filter(camera == "SWIR") |> 
...
```{r}
smoothed_hyperspec_2021 <- VNIR_smoothed_padded_out |> 
...
```{r}
VNIR_cut <- smoothed_hyperspec_2021 |> filter(wavelength > 400, wavelength < 1001, camera == "VNIR")
SWIR_cut <- smoothed_hyperspec_2021 |> filter(wavelength >1000, wavelength < 2500, camera == "SWIR")
smoothed_hyperspec_2021_truncated <- rbind(VNIR_cut, SWIR_cut)
```
...
```{r}
smoothed_wide <- smoothed_hyperspec_2021_truncated |> 
...
unsmoothed_wide <- smoothed_hyperspec_2021_truncated |> 
  filter(wavelength >994, wavelength < 1001) |>
...
```{r}
correction_1002.18 <- smoothed_hyperspec_2021_truncated |> 
...
corrected_hyperspec_2021 <- VNIR_cut |> 
...
```{r}
corrected_smoothed_wide <- corrected_hyperspec_2021 |>  
...
corrected_raw_wide <- corrected_hyperspec_2021 |>  
...
```{r}
write_csv(corrected_hyperspec_2021, "out/2021/raw_and_smoothed_full.csv")
write_csv(FWHM_binned, "out/2021/FWHM_binned.csv")
write_csv(double_FWHM_binned, "out/2021/double_FWHM_binned.csv")
```
