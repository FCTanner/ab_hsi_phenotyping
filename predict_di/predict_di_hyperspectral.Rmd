...
```{r}
year_to_run = "2020"
scoring_date = as.Date("2020-06-09")
```
...
* Still to do:
  * Include more models
  * Tune LASSO parameters on bootstrapped dataset 
...
dataset_iterator_no_der <- expand.grid(imaging_date = scoring_date,
                                       alive_only = c(TRUE), 
...
dataset_iterator_derivatives <- expand.grid(imaging_date = scoring_date,
                                            alive_only = c(TRUE), 
...
run_cv <- function(input = prep_dat_out, output, cv_folds = n_folds, cv_repeats = n_repeats, random_seed = seed){
  ...
  folds_for_cv <- vfold_cv(input, v = cv_folds, repeats = cv_repeats, strata = di)
...
```{r}
write_csv(cv_metrics, paste("out/hyperspectral_cv_metrics_", year_to_run, ".csv", sep = ""))
```
